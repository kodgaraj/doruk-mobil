    ...
    <View style={{ flex: 1, padding: 5 }}>
      <ScrollView style={{ marginBottom: 50 }}>
        <List.Section key={"siparis-listesi"}>
          {siparisler.yukleniyor ? (<Loading key={"loading"} />) : (<>
              ...
              return (
                <View key={index + "view"}>
                  <List.Accordion
                    ...
                    left={props => <Avatar.Text key={index + "avatar"} label={siparis.siparisId} size={36} />}
                    style={styles.accordionBg}>
                    <List.Item
                      key={index + "siparis-no"}
                      title="Sipariş No"
                      style={styles.accordionBg}
                      right={props => <Text key={index + "siparis-no-text"} style={styles.itemText}>{siparis.siparisNo}</Text>} />
                    <List.Item
                      key={index + "siparis-durumu"}
                      title="Sipariş Durumu"
                      style={styles.accordionBg}
                      right={props => <Text key={index + "siparis-durumu-text"} style={styles.itemText}>{siparis.siparisDurumAdi}</Text>} />
                    <List.Item
                      key={index + "irsaliye-no"}
                      title="İrsaliye No"
                      style={styles.accordionBg}
                      right={props => <Text key={index + "irsaliye-no-text"} style={styles.itemText}>{siparis.irsaliyeNo}</Text>} />
                    <List.Item
                      key={index + "tutar"}
                      title="Tutar"
                      style={styles.accordionBg}
                      right={props => <Text key={index + "tutar-text"} style={styles.itemText}>{siparis.tutar} ₺</Text>} />
                    <List.Item
                      key={index + "gecen-sure"}
                      title="Geçen Süre"
                      style={styles.accordionBg}
                      right={props => <Badge key={index + "gecen-sure-badge"} size={30} style={{ backgroundColor: theme.colors[siparis.gecenSureRenk], ...styles.itemBadge }}>{siparis.gecenSure + " gün"}</Badge>} />
                    <List.Item
                      key={index + "islem-sayisi"}
                      onPress={() => { }}
                      title="İşlem Sayısı"
                      style={styles.accordionBg}
                      right={props => <Button key={index + "istem-sayisi-buton"} contentStyle={{ flexDirection: 'row-reverse' }} icon="eye" mode="contained-tonal" style={styles.itemText}>{siparis.islemSayisi} adet </Button>} />
                  </List.Accordion>
